...
**本项目现在使用自托管 PostgreSQL + Node.js API 方案**，不依赖 Supabase。数据完全在你自己的服务器上，无第三方认证，邮箱/密码由项目内置 `bcryptjs + JWT` 处理。

> **历史背景（可跳过）：** 项目早期曾评估过 Supabase 免费版作为 BaaS 方案，对家庭小规模使用在容量上足够；后来出于「数据在国内、可控、无邮箱验证依赖」等考量迁移到自托管腾讯云方案，见本文末「附：从外部 BaaS 迁移」。
...
对「只要国内访问、数据自控」的家庭用户，此方案比托管型云数据库更直观可控。
