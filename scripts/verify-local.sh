...
guard_storage_scope() {
  ...
  local violations
  violations="$(printf '%s\n' "$matches" | rg -v "^(src/lib/api/http.ts|src/lib/communityListCache.ts|src/lib/mealPhotoGuide.ts|src/lib/telemetry.ts|src/lib/themePersistence.ts|src/lib/__tests__/themePersistence.test.ts)$" || true)"
  if [ -n "$violations" ]; then
