...
export function CommunityMemberCard({ member, todayKey }: CommunityMemberCardProps) {
  ...
  return (
    <Link
      ...
      className="group block rounded-2xl bg-card/80 p-4 ring-1 ring-slate-700/50 transition hover:ring-violet-500/40 hover:bg-slate-800/90 active:scale-[0.99]"
    >
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate font-semibold text-slate-100">
            {member.nickname}
            {member.isSelf && (
              <span className="ml-1.5 text-xs font-normal text-violet-300">我</span>
            )}
          </p>
        ...
        <div className="text-right">
          <p
            className={`text-lg font-bold tabular-nums ${
              surplus ? 'text-red-400' : deficit > 0 ? 'text-emerald-400' : 'text-amber-400'
            }`}
          >
            {deficit > 0 ? '+' : ''}
            {Math.round(deficit)}
      ...
      <p className="mt-2 text-xs text-violet-300/80 opacity-0 transition group-hover:opacity-100">
        查看打卡墙与记录 →
      </p>
    </Link>
  )
