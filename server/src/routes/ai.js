...
router.post(
  ...
  authMiddleware,
  asyncHandler(async (req, res) => {
    const { type, description, modality, image } = req.body
    ...
    if (modality === 'image') {
      ...
      let consumed = false
      try {
        await assertMealPhotoQuotaAvailable(req.userId, req.userEmail)
        consumed = await tryConsumeMealPhotoQuota(req.userId, req.userEmail)
        if (!consumed) {
          const quota = await getMealPhotoQuota(req.userId, req.userEmail)
        ...
        const result = await visionEstimator({
          ...
          modality: 'image',
          imageDataUrl: typeof image === 'string' ? image : '',
          profile,
        })
        ...
        if (consumed) {
          await refundMealPhotoQuota(req.userId, req.userEmail)
        }
